...
export function getRegistryComponent(name: string) {
  return Index["8starlabs-ui"][name]?.component;
}

export async function getRegistryItem(name: string) {
  const item = Index["8starlabs-ui"][name];
...
function buildPathMappings(
  ...
  files.forEach((file) => {
    if (file.path && file.target) {
      // Extract the source path relative to registry/8starlabs-ui/
      const match = file.path.match(/registry\/8starlabs-ui\/(.+)$/);
      if (match) {
...
export function fixImport(content: string, pathMappings: Map<string, string>) {
  ...
  // Fix hook imports
  content = content.replaceAll("@/registry/8starlabs-ui/hooks/", "@/hooks/");

  // Fix lib imports
  content = content.replaceAll("@/registry/8starlabs-ui/lib/", "@/lib/");
